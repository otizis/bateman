...
public class YahooQuoteFetcher extends QuoteFetcher
{
    ...
    public TimeSeries fetchAndParseDaily(String symbol, int days)
        throws Exception
    {
        return fetchAndParse(symbol, days, 60 * 60 * 24);
    }
    
    public BigDecimal fetchBidAskSpread(String symbol)
        throws Exception
    {
    ...
    public List<Quote> parseQuotes(String quoteList, int interval)
    {
        ...
        for (String line : lines)
        {
            ...
            Quote quote = new Quote(date, interval, new BigDecimal(parts[1]), new BigDecimal(parts[2]),
                new BigDecimal(parts[3]), new BigDecimal(parts[6]), Integer.parseInt(parts[5]));
